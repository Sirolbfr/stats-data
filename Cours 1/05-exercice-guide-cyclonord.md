...
| `Magasin`           | Qualitative nominale                                                                           | Non                          |
...
| `Quantite`          | Quantitative discrète                                                                          | Oui                          |
...
>  -> La moyenne de la note n'est pas vraiment pertinente à calculer, la médiane le serait déjà beaucoup plus. La somme des notes clients n'ayant pas de sens, la moyenne n'en a pas non plus.
...
Grâce à l'écart entre la moyenne et la médiane, on peut dire que les valeurs de Montant et Prix unitaire sont étalées vers la droite (quelques valeurs excessivement hautes); quant à la distribution des notes clients, on peut dire qu'elle est presque symétrique ou normale.
...
Cette phrase manque de précision, certes la moyenne des notes client est bien de 4.10 mais cette moyenne porte seulement sur les 560 commandes possédant une note client.
*« Nos 560 commandes ayant reçu une note client obtiennent une note moyenne de 4,1/5. »* 
...
Le million de chiffre d'affaire représente la totalité des commandes répertoriées, qu'elles aient été traitées ou non, qu'elles aient été retournées ou annulées ou non. Ce chiffre n'est pas pertinent du tout.
...
> Les commandes VAE représentent un gros pourcentage du montant total tout en représentant un petit pourcentage du nombre de commandes car le montant moyen de chaque commande VAE est beaucoup plus élevé que dans les autres catégories.
...
La moyenne pondérée accorde plus d'importance aux commandes représentant un montant TTC plus important. On peut corriger la formule de la moyenne pondérée en y ajoutant des conditions : on peut ignorer les lignes qui ne comportent pas de note client.

`=SUMPRODUCT((T_Ventes[Note_client] > 0) * T_Ventes[Note_client]; T_Ventes[Montant_TTC]) / SUMIF (T_Ventes[Note_client]; ">0"; T_Ventes[Montant_TTC])`
